	...
	public var printableWordWithColoredUnguessedLetters:NSAttributedString
	{
		get
		{
			...
			for (i, c) in self.printableWord.enumerated() {
				if self.guessedLetters.contains(c) {
					coloredWord.addAttribute(NSAttributedString.Key.foregroundColor, value:UIColor.systemGreen, range:NSMakeRange(i, 1))
				} else {
					coloredWord.addAttribute(NSAttributedString.Key.foregroundColor, value:UIColor.systemRed, range:NSMakeRange(i, 1))
				}
			}
